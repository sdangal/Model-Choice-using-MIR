...
List fopls(arma::mat X, 
  ...
  return Rcpp::List::create(
	...
	Rcpp::Named("scores") = scores,
    Rcpp::Named("X.loadings") = Xloadings,
	Rcpp::Named("Y.loadings") = Yloadings,   //added by Shree
    Rcpp::Named("projectionM") = prjM,
    Rcpp::Named("transf") = Rcpp::List::create(
      Rcpp::Named("Xcenter") = xfcntr,
      Rcpp::Named("Xscale") = Xs
...
Rcpp::NumericMatrix predopls(arma::mat bo, 
  ...
  //std::cout << predicted<< " "<<Xz.n_rows<< " "<<repmat(bo.cols(0,ncomp-1), Xz.n_rows,1)<< " " ;
  //std::cout<<Xz.n_rows<<" " ;
    ...
    for(int i = 0; (unsigned)i < mindices.n_cols; i++){
      ...
      ypred = Rcpp::as<arma::mat>(predopls(fit["bo"], 
                                           fit["coefficients"], 
                                              ...
                                              scale,
                                              transf["Xscale"]));
	  //std::cout << "ypred"<<" " <<ypred << " " << "pxmatslice"<< " "<<pxmatslice<< " " << mindices.col(i) << " " <<"index"<< " " <<mindices<< " " << pindices;
	  //std::cout <<ncomp<<" " << "ypred"<<" " <<ypred(1,5);
	  ...
	  //std::cout<< "ypred"<< " " << ypred << " " <<rmseseg.col(i)<< " " <<rpymatslice << " " <<mindices.col(i);
    }
  }
