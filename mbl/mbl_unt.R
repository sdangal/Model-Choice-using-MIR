...
mbl <- function(Yr, Xr, Yu = NULL, Xu,
  ...
  #added by Shree Dangal 03/28/2018
  #bug where a single validation sample was not predicted has been fixed below
  #The error was due to conversion of d.mat to vector when only one sample was used
  if(nrow(k_mat) < 2){
  d.mat <- as.matrix(d.mat, ncol=1)
  }
  ##Finished bug fix
  ...
  predobs <- foreach(tmp.val = iter(val, by = "row"), kn.org = iter(k.org, by = "row"), i = icount(),
                                   ...
                                   for(kk in 1:length(it$k))
                                   {
                                    ...
                                    # pb <- txtProgressBar(width = 10, char = "-")
                                     ...
                                     if(knkk != ifelse(kk == 1, 0, it$k[kk-1])){
									   ...
									   index.s <- dk
									   calib.s <- data.frame(Yr[dk])
									   dist.s <- data.frame(it$d[dk])
									   #predobs$coeff <- matrix("NA", nrow = length(it$k), ncol = ncol(Xr))
									  ...
									  #cat(index.s, '\n')	
                                       ...
                                       i.pred <- locFitnpred(x = tmp.cal$mat, y = tmp.cal$y, 
										...
										best.pls <- i.pred$validation$bestpls.c
										predobs$bo[kk] <- int.bo[best.pls]	
										predobs$ok[kk] <- list(i.pred$validation$models$coefficients[,best.pls])
										predobs$load[kk] <- list(i.pred$validation$models$X.loadings[best.pls,])										
										predobs$calib.val[kk] <- calib.s
										predobs$dist.s[kk] <- dist.s
										predobs$row.index[kk] <- data.frame(index.s)
										samples.close$samples.id[i] <- predobs$row.index[kk]
										samples.close$samples.dist[i] <- predobs$dist.s[kk]
										samples.close$samples.calib[i] <- predobs$calib.val[kk]
										
										#start calculating prediction error - similar to ydev in Unscrambler -- actual prediction is done in line 1092-1107
										
										#define required matrix and datasets
										orig.X <- tmp.cal$mat
										Xmeans.cal <- matrix(colMeans(orig.X[,,drop=FALSE]), nrow=1)
										x.proj <- i.pred$validation$models$projectionM
										...
										ta.calib <- diag(crossprod(x.scores))
										ta.valid <- valid.TT^2
										predobs$lev[kk] <- ta.valid[1,best.pls] / ta.calib[best.pls]
			...
			predobs$coeff <- 	do.call(rbind, predobs$ok)
			predobs$loadings <- do.call(rbind, predobs$load)
			predobs$ok <- NULL
			predobs$load <- NULL
			predobs$row.index <- NULL
			predobs$dist.s <- NULL
			predobs$calib.val <- NULL
...
#added by Shree Dangal 04/10/2018 -- to provide uncertainty estimates for new samples based on N nearest neighbor 
  ...
  resultsList <- list(call = call.f,
					  ...
					  neighbors.stat = list(samples.id = do.call(rbind, samples.close$samples.id),samples.dist=do.call(rbind,samples.close$samples.dist), samples.calib =do.call(rbind,samples.close$samples.calib)), 
  ...
  return(resultsList)
  predobs[max.neigh][[1]][["row.index"]] <- NULL
}
...
locFitnpred <- function(x, y, predMethod, scaled = TRUE, weights = NULL, newdata, 
  ...
  if(predMethod == "pls"){
	...
	#cat(str(fit$coefficients))
	
	### make new predictions internally by using coefficients and intercept 			//added by shree dangal 03/28/2018
	#cat(newdata %*% fit$coefficients + rep(fit$bo, each= nrow(newdata)), '\n')
  }
  ...
  #print(names(results$validation$models))
  #print((results$validation$models))
